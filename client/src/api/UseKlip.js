...
import { NFT_MARKET_CONTRACT_ADDRESS, NFT_SIMPLE_CONTRACT_ADDRESS } from "../constants";

const A2P_API_PREPARE_URL = 'https://a2a-api.klipwallet.com/v2/a2a/prepare';
const APP_NAME = "KLAY_MARKET";
const isMoblie = window.screen.width >= 1200 ? false : true;
...
export const buyCard = async (
	...
	executeContract(NFT_MARKET_CONTRACT_ADDRESS, functionJson, "10000000000000000", `["${tokenId}", "${NFT_SIMPLE_CONTRACT_ADDRESS}"]`, setQrvalue, callback);
};
...
export const listingCard = async (
	...
	executeContract(NFT_SIMPLE_CONTRACT_ADDRESS, functionJson, "0", `["${fromAddress}", "${NFT_MARKET_CONTRACT_ADDRESS}", "${tokenId}"]`, setQrvalue, callback);
};
...
export const mintCardWithURI = async (
	...
	executeContract(NFT_SIMPLE_CONTRACT_ADDRESS, functionJson, "0", `["${toAddress}", "${tokenId}", "${uri}"]`, setQrvalue, callback);
	console.log(toAddress, tokenId, uri);
};
